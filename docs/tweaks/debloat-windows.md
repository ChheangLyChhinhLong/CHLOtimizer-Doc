...
- **Description**: Choose between CHLOtimizer Debloat script or Raphire's Win11Debloat script to remove built-in Windows apps and bloatware.
...
## 1. CHLOtimizer Custom Selective Script:
...
if (-not (Test-IsAdmin)) {
    Write-Host "[CHLOtimizer Debloat] This script must be run as Administrator." -ForegroundColor Red
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="CHLOtimizer Debloat v$version"
    ...
    <Grid Margin="24">
        ...
        <Border Grid.Row="1" 
                ...
                Margin="0,0,0,20">
            <StackPanel>
                <RadioButton x:Name="RadioCHLOtimizer"
                            Content="CHLOtimizer Debloat (Choose which apps to remove) - Recommended"
                            Margin="0,0,0,16" 
                            IsChecked="True"/>
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="CHLOtimizer Debloat - Important Change"
...
function Remove-SelectedApps {
    ...
    Write-Host "Starting CHLOtimizer debloat..." -ForegroundColor Green
    ...
    Write-Host "CHLOtimizer debloat completed!" -ForegroundColor Green
}

try {
    $script:appsWereRemoved = $false
    
    Write-Host "Starting CHLOtimizer Debloat script..." -ForegroundColor Green
    ...
    elseif ($ScriptChoice -eq "custom") {
        if ($AppsToRemove.Count -gt 0) {
            Write-Host "Running CHLOtimizer debloat to remove $($AppsToRemove.Count) apps..." -ForegroundColor Green
