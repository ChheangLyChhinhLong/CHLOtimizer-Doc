...
Tweaks are small, targeted changes to Windows settings, registry keys, or system services that modify how your PC behaves. In CHLOtimizer, each tweak is a self-contained PowerShell script that applies a specific optimization or customization.
...
When you toggle a tweak on in CHLOtimizer, it runs the `apply.ps1` script. When you toggle it off, it runs `unapply.ps1`.
...
## Risk Levels <span style="color: #ff9800;">(CHLOtimizer 2.16.0+  Required)</span>
...
CHLOtimizer provides several safeguards when using tweaks:

- **System Restore Points** — Always create a restore point before applying tweaks. CHLOtimizer supports this on the home page.
...
## Reapplying Tweaks <span style="color: #ff9800;">(CHLOtimizer 2.18.0+  Required)</span>
...
CHLOtimizer's tweak system is open and extensible. If you want to create a custom tweak, see the [Creating Tweaks](/creating-tweaks) guide for the full specification and directory structure.
